...
// TODO: fix the prompt. Issues - Prompt is displayed before the output is thrown to stdout.
...
cli.responders.help = function() {
  var commands = {
    ...
    'list checks --up --down': 'Show list of all the active checks in the system, including their state. The "--up" and "--down" flags are both optional',
    'more checks info --{checkId}': 'Show details of a specified check',
    'list logs': 'Show a list of all the log files avaliable to be read (Compressed Only)',
    'more logs info --{fileName}': 'Show details of a specified log file'
  };
      ...
      checkIds.forEach(function(checkId) {
        _data.read('checks', checkId, function(err, checkData) {
          if (!err && checkData) {
            ...
            var stateOrUnkown = typeof (checkData.state) === 'string' ? checkData.state : 'unknown';
  
            // If the user has specified the state or hasn't specified any state include current check accordingly
            if (lowerString.indexOf('--'+state) > -1 || (lowerString.indexOf('--down') === -1 && lowerString.indexOf('--up') === -1)) {
              var line = 'ID: ' + checkData.id + ' ' + checkData.method.toUpperCase() + ' ' + checkData.protocol+'://'+checkData.url + ' State: ' + stateOrUnkown;
              console.log(line);
              cli.verticalSpace(1);
            }
          }
...
cli.responders.listLogs = function () {
  // Using our custom list function
  ...
  ls.stdout.on('data', function(dataObject) {
    ...
    logFileNames.forEach(function(fileName) {
      if (typeof (fileName) === 'string' && fileName.length > 0 && fileName.indexOf('-') > -1) {
...
cli.responders.moreLogInfo = function (str) {
  ...
  if (str) {
    ...
    var arr = str.split('--');
    var fileId = typeof(arr[1]) === 'string' && arr[1].trim().length > 0 ? arr[1].trim() : false;
    if (fileId) {
      cli.verticalSpace(1);
      // Decompress the log file
      _logs.decompress(fileId, function(err, strData) {
        if (!err && strData) {
         // Split into lines
  ...
  if (str) {
    ...
    uniqueInputs.some(function(input) {
      if (str.toLowerCase().indexOf(input) > -1) {
        matchFound = true;
        // Emit an event matching the unqiue input, and include the string given by the user
        e.emit(input, str);
        return true;
      }
    });
